...
function OnGovernmentHover()
	...
	local Civic2 = "[COLOR_NEGATIVE_TEXT]Requires Military Theory (Archery)[ENDCOLOR]"
...
function OnLaborHover()
	...
	if (Teams[pPlayer:GetTeam()]:IsHasTech(GameInfoTypes.TECH_ROBOTICS)) then
		Civic6 = "[COLOR_POSITIVE_TEXT]Available[ENDCOLOR]"
	end
...
function OnEconomyHover()
	local pPlayer = Players[Game.GetActivePlayer()]
	local Civic1 = "[COLOR_POSITIVE_TEXT]Available[ENDCOLOR]"
	local Civic2 = "[COLOR_NEGATIVE_TEXT]Requires Trade (Horseback Riding)[ENDCOLOR]"
	local Civic3 = "[COLOR_NEGATIVE_TEXT]Requires Currency[ENDCOLOR]"
	local Civic4 = "[COLOR_NEGATIVE_TEXT]Requires Economics[ENDCOLOR]"
	local Civic5 = "[COLOR_NEGATIVE_TEXT]Requires Scientific Theory[ENDCOLOR]"
	local Civic6 = "[COLOR_NEGATIVE_TEXT]Requires Corporations[ENDCOLOR]"
	local Civic7 = "[COLOR_NEGATIVE_TEXT]Requires Ecology[ENDCOLOR]"
	if pPlayer:GetCurrentEra() > 2 then
		Civic1 = "[COLOR_NEGATIVE_TEXT]Obsoleted[ENDCOLOR]"
	end
	...
	if (Teams[pPlayer:GetTeam()]:IsHasTech(GameInfoTypes.TECH_SCIENTIFIC_THEORY)) then
		Civic5 = "[COLOR_POSITIVE_TEXT]Available[ENDCOLOR]"
	end
...
function OnCivicSelected(playerID)
	...
	elseif GovernmentName == "Totalitarianism" or GovernmentName == "National Suffrage" then GovernmentMaintain = pPlayer:GetBaseBuildingMaintenance()*0.25 end
	...
	elseif LegalName == "Feudalism" or LegalName == "Bureaucracy" or LegalName == "Constutionalism" then LegalMaintain = pPlayer:GetBaseBuildingMaintenance()*0.24 end
	...
	elseif LaborName == "Industrialized Servitude" or LaborName == "Unionism" then LaborMaintain = pPlayer:GetBaseBuildingMaintenance()*0.23 end
	...
	elseif EconomyName == "Corporatism" or EconomyName == "Environmentalism" then EconomyMaintain = pPlayer:GetBaseBuildingMaintenance()*0.22 end
	...
	elseif ReligionName == "Theocracy" or ReligionName == "Cult of Personality" or ReligionName == "Free Religion" then ReligionMaintain = pPlayer:GetBaseBuildingMaintenance()*0.11
	elseif ReligionName == "Organized Faith" then ReligionMaintain = pPlayer:GetBaseBuildingMaintenance()*0.21 end
